...
#cp is call or put. values c or p
cp = 'c'

#f forward price f = spot price now * e^(rate*time_to_expiry_in_years)
f = 411*exp(.06*.052054794520548)

#k is strike price
k = 411

#t is time to maturity (expiry?) in years so 1/365
t = .052054794520548

#r is interest rate
r = .028

#v is implied volatility
# v = .2876

#comp is number of times compounded. we can use inf from numpy which i assume is infinite (constant)
comp = inf

#full is True or false.  True gives dictionary with price and all else. Else only calculated param returned
full = True
...
def straddle(spot, strike, dte, ivcrushed, optprice):
    t = dte/365
    k = strike
    r = .0289
    f = spot*exp(t*r)
    ...
    price = optprice
    cp = 'c'
    call = black(cp, f, k, t, r, v, price, full=False, comp=inf)
    cp = 'p'
    put = black(cp, f, k, t, r, v, price, full=False, comp=inf)
    return(call+put)
# print(straddle(callput, spot, strike, dte, rfrate, ivcrushed, optprice))
